...
CSpriteUI::SPRITE_STATE CResourceSpriteUI::GetSpriteUIState(CResourceSpriteUI::enSpriteUI SpriteUI) 
{
	...
	//全SpriteUIの共通初期設定.
	for (int spriteUI = 0; spriteUI < static_cast<int>(enSpriteUI::Max); spriteUI++) {
		//時間番号だけ例外処理.
		if (spriteUI == static_cast<int>(enSpriteUI::TimerNum)) {
			continue;
		}

		m_pSpriteUI_State[spriteUI].Stride.w = m_pSpriteUI_State[spriteUI].Base.w;
		m_pSpriteUI_State[spriteUI].Stride.h = m_pSpriteUI_State[spriteUI].Base.h;
	}
