...
public:
	...
	const D3DXVECTOR2	GAGE_POS			= D3DXVECTOR2(400.0f, 550.0f);		//ゲージの位置.
	...
	//const int			BARRAGE_MAX			= 3;								//連打の最大数.
...
private:
	...
	void Time();										//時間処理判定.
	void TimeRender();									//時間描画処理関数.
	void ButtonChange();								//ボタン変更処理関数.
	void ButtonRender();								//ボタンの描画.
	void GageRender();									//ゲージの描画.
	void PushButtonRender();							//ボタンを押したときの描画処理関数.
	void DispDecision();								
